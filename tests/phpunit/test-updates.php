...
class Test_Updates extends WP_UnitTestCase {
	...
	/**
	 * "Tested up to 7.1" covers 7.1.2, so WordPress does not call the plugin
	...
	/**
	 * The author link on the Plugins screen opens clsdir.com in a new tab and
	 * says so to a screen reader. Other plugins' rows are left alone.
	 */
	public function test_the_author_link_opens_in_a_new_tab(): void {
		$core = array( 'Version 1', 'By <a href="https://www.clsdir.com/">CLSDIR</a>' );

		$ours   = Updates\author_opens_new_tab( $core, Updates\basename_key() );
		$theirs = Updates\author_opens_new_tab( $core, 'other/other.php' );

		$this->assertStringContainsString( 'href="https://www.clsdir.com/" target="_blank" rel="noopener"', $ours[1] );
		$this->assertStringContainsString( '<span class="screen-reader-text"> (opens in a new tab)</span>', $ours[1] );
		$this->assertSame( $core, $theirs );
	}
