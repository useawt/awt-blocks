...
* [A11y] A Modal with no second button no longer draws an empty one. Clearing the "Secondary action label" left a button with nothing in it, which a screen reader announces as just "button" and a keyboard user tabs into for no reason. Leave the label empty and the button is gone; the setting now says so. Modals with a label are unchanged, down to the whitespace.
