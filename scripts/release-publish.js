...
function main() {
	...
	if ( pkg.version !== version ) {
		fail(
			`package.json says ${ pkg.version }, you asked for ${ version }.`
		);
	}
	...
	const newest = ( changelog.match( /^## (?!Unreleased$)(.+)$/m ) ||
		[] )[ 1 ];
	...
	if ( ! firstLine.startsWith( `## ${ version }` ) ) {
		fail(
			`RELEASE_NOTES.md opens with "${ firstLine }", not ## ${ version }.`
		);
	}

	const disagreements = versionSightings().filter(
		( s ) => s.says !== version
	);
	...
	run(
		`git tag -a ${ tag } -m ${ JSON.stringify(
			`${ pkg.name } ${ version }`
		) }`,
		dryRun
	);
